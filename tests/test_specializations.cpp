...
namespace bitpacker {
    template <>
    constexpr void store(span<byte_type> buffer, std::size_t offset, std::complex<double> value) noexcept {
    ...
    template <>
    constexpr std::complex<double> get(span<const byte_type> buffer, std::size_t offset) noexcept {
...
TEST_CASE("Specialize pack/unpack on a complex type", "[pack][unpack][specialize]") {
    std::array<uint8_t, 9> buff{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    const std::complex input{4.0, 8.0};
    bitpacker::store(buff, 4, input);
