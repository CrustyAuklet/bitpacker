...
namespace bitpacker {
    ...
    namespace impl {
        ...
        constexpr Offset get_offset(size_type bit_offset) noexcept {
            return {(bit_offset / ByteSize), (bit_offset % ByteSize)};
        }
        ...
        using unsigned_type = std::conditional_t<
            NumBits <= 8, uint8_t,
            std::conditional_t<NumBits <= 16, uint16_t, std::conditional_t<NumBits <= 32, uint32_t, std::conditional_t<NumBits <= 64, uint64_t, void> > > >;
