...
namespace bitpacker {
    namespace impl {
        ...
        constexpr bool isFormatType(char formatChar) noexcept {
            return formatChar == 'u' || formatChar == 's' || formatChar == 'f' || formatChar == 'b' || formatChar == 't' || formatChar == 'r' || formatChar == 'p' ||
                   formatChar == 'P';
        }
                ...
                std::conditional_t<FormatChar == 'f', float,
                                   std::conditional_t<FormatChar == 'b', bool,
                                                      std::conditional_t<FormatChar == 't', std::array<char, bit2byte(BitCount)>,
                                                                         std::conditional_t<FormatChar == 'r', std::array<byte_type, bit2byte(BitCount)>, void>>>>>>;
        ...
        constexpr auto unpackElement(span<const byte_type> buffer, size_type offset) -> typename UnpackedType::return_type {
            ...
            if constexpr (UnpackedType::format == 'f') {
                static_assert(UnpackedType::bits == 16 || UnpackedType::bits == 32 || UnpackedType::bits == 64, "Expected float size of 16, 32, or 64 bits");
            }
        ...
        constexpr int packElement(span<byte_type> buffer, size_type offset, InputType elem) {
            ...
            if constexpr (PackedType::format == 'f') {
                static_assert(PackedType::bits == 16 || PackedType::bits == 32 || PackedType::bits == 64, "Expected float size of 16, 32, or 64 bits");
            }
        ...
        constexpr auto insert_padding(span<byte_type> buffer, const size_type start_bit, std::index_sequence<Items...> /*unused*/) {
            ...
            using FormatTypes = std::tuple<typename impl::FormatType<formats_only_pad[Items].formatChar, formats_only_pad[Items].count, formats_only_pad[Items].endian>...>;
        ...
        constexpr void pack(std::array<byte_type, N>& output, const size_type start_bit, std::index_sequence<Items...> /*unused*/, Args&&... args) {
            ...
            using FormatTypes = std::tuple<typename impl::FormatType<formats_no_pad[Items].formatChar, formats_no_pad[Items].count, formats_no_pad[Items].endian>...>;
    ...
    constexpr auto unpack(Fmt /*unused*/, Input&& packedInput) {
        return impl::unpack<Fmt>(std::make_index_sequence<impl::count_non_padding(Fmt{})>(), std::forward<Input>(packedInput), 0);
    }
    ...
    constexpr auto unpack_from(Fmt /*unused*/, Input&& packedInput, const size_type offset) {
        return impl::unpack<Fmt>(std::make_index_sequence<impl::count_non_padding(Fmt{})>(), std::forward<Input>(packedInput), offset);
    }
    ...
    constexpr auto impl::unpack(std::index_sequence<Items...> /*unused*/, Input&& packedInput, const size_t start_bit) {
        ...
        const auto unpacked = std::make_tuple(impl::unpackElement<typename std::tuple_element_t<Items, FormatTypes>>(
            // NOLINTNEXTLINE - this is the standard implementation of std::as_bytes() from c++20
            {reinterpret_cast<const byte_type*>(std::data(packedInput)), std::size(packedInput)}, formats[Items].offset + start_bit)...);
        return unpacked;
    }
...
#define BP_STRING(s)                                           \
    [] {                                                       \
        struct S : bitpacker::impl::format_string {            \
            ...
            static constexpr bitpacker::size_type size() {     \
                return std::size(value()) - 1;                 \
            }                                                  \
            static constexpr auto at(bitpacker::size_type i) { \
                return value()[i];                             \
            };                                                 \
        };                                                     \
        return S{};                                            \
